...
impl HTTPClient {
  ...
  pub fn new(uri: &str, headers: Option<HeaderMap>) -> Result<HTTPClient, Error> {
    let uri = Url::parse(uri).map_err(Error::from)?;

    let mut default_headers = HTTPClient::get_default_headers();
  ...
  async fn get_client(&self, reset_headers: bool) -> Arc<RwLock<ClientWrapper>> {
    if reset_headers {
      self.client.write().await.set_new_headers(HTTPClient::get_default_headers());
    }

    self.client.clone()
  }

  async fn act_internal(&mut self, method: Method, route: &str, body: Option<&impl Serialize>)
  ...
  async fn act<T: for<'de> serde::Deserialize<'de>>(&mut self, method: Method, route: &str, body: Option<&impl Serialize>)
